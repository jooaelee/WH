...
3. 압축을 푼 폴더에서
   ...
   - **Mac**: `실행하기_Mac.command` 더블클릭 (macOS가 "확인되지 않은 개발자" 경고를 띄우면,
     파일에서 마우스 우클릭 → **열기** → **열기** 확인)
4. 검은 창이 뜨고 최초 1회에 한해 필요한 프로그램을 자동으로 설치합니다 (몇 분 소요). 이후에는
   바로 실행됩니다.
5. 잠시 후 **브라우저가 자동으로 열리며** 업로드 화면이 나타납니다. 출고·입고 CSV 파일을
   올리고 **[🚀 예측 실행]** 을 누르면 됩니다.
6. 다음에 또 쓸 때도 같은 파일을 더블클릭하면 됩니다 — 검은 창을 닫으면 프로그램이 종료됩니다.
